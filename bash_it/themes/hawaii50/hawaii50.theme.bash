...
# SCM prompts
SCM_THEME_PROMPT_DIRTY="${green}!${normal}"
...
function virtual_prompt_info() {
    ...
    [[ -n "$virtual_env_info" ]] && virtual_prompt="${VE_COLOR}($virtual_env_info)${DEFAULT_COLOR}"
...
function prompt() {
    ...
    if [[ $VIRTUAL_PROMPT_ENABLED == 1 ]]; then
        PS1="${UC}\u ${DEFAULT_COLOR}at ${MACHINE_COLOR}\h$(ip_prompt_info) ${DEFAULT_COLOR}in ${DIRECTORY_COLOR}$(limited_pwd)${DEFAULT_COLOR}$(virtual_prompt_info)$(scm_prompt_info)${reset_color} \$(scm_char) "
    else
        PS1="${UC}\u ${DEFAULT_COLOR}at ${MACHINE_COLOR}\h$(ip_prompt_info) ${DEFAULT_COLOR}in ${DIRECTORY_COLOR}$(limited_pwd)${DEFAULT_COLOR}$(scm_prompt_info)${reset_color} \$(scm_char) "
    fi
    PS2='> '
    PS4='+ '
}
